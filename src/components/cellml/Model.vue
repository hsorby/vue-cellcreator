<template>
  ...
  >
    <div class="display-flex entity-menus-container" :style="size">
      <clicktoedit
        v-model="name"
        class="self-aligned-start"
        default-value="set model name here"
      ></clicktoedit>
      <slidingmenu
        class="menu"
        tag="div"
    ...
    <div class="model-children display-flex">
      <div class="units-children">
        <div
          ...
          v-for="(units, unitsIndex) in getUnits(index)"
      ...
      <ul class="component-children">
        <li
          ...
          v-for="(component, componentIndex) in getComponents(index, [])"
...
export default {
  ...
  methods: {
    ...
    _dropUnits(event) {
      ...
      if (event.dataTransfer.effectAllowed === "copy" && index !== "") {
        ...
        const sourceIndex = parseInt(
          event.dataTransfer.getData("int/model-index")
        );
        const payload = {
          sourceModel: sourceIndex,
          targetModel: this.index,
          unitsToClone: parseInt(index),
        };
    ...
    _dropComponent(event) {
      const sourceIndexPathStr = event.dataTransfer.getData(
        "int/component-index-path"
      );
      if (
        ...
        const targetIndexPathStr = JSON.stringify(
          targetIndexPath.slice(0, sourceIndexPath.length)
        );
        if (
          !(
            sourceIndex === this.index &&
            sourceIndexPathStr === targetIndexPathStr
          )
        ) {
          const sourceIndexPath = JSON.parse(sourceIndexPathStr);
          const payload = {
        ...
        // Adding new
        this.$store.commit("models/addNewComponent", {
          targetModel: this.index,
          targetPath: [],
        });
      }
